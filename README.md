...
## I. Proposal (Option 1)
### Motivation : 
음성 감정 인식은 사람들의 심리적 상태를 이해하고 문제를 해결하는 데에 도움을 줄 수 있다. 

가령 우울증이나 불안장애를 가진 사람들에게 도움을 주고 스트레스나 분노로 인한 갈등을 미리 예방하는 데에도 도움을 줄 수 있다. 

또한, 상업적인 관점에서도 고객의 음성을 통해 감정과 반응을 파악함으로써 기업의 제품 및 서비스의 개선방안을 설정할 수 있다. 

음성 데이터를 다루는 것이 비교적 생소하지만, 수업 시간에 배운 딥러닝 기법을 이용하여 음성 데이터를 분석해보고 싶어 해당 주제를 선택하게 되었다.


### What do you want to see at the end?
어떤 음향 데이터든 감정을 정확하게 인식하는 모델을 만들고 싶다.
...
------------------------------------
## III. Methodology
> 크게 두가지 방식으로 진행하였다. 첫번째로 WAV2VEC을 이용하였고, 두번째로 LIBROSA를 이용하였다.
### [1] Wav2vec 2.0
...
#### submission.csv가 위와 같이 1880개의 데이터셋을 테스트 하였고 데이콘에 제출한 결과 wav2vec을 이용한 정확도는 약 0.32, librosa를 이용한 정확도는 약 0.45으로 나왔다. 찍었을때 확률이 0.16161..임을 감안하면 약 2.8배 정확한 걸 알 수 있다. 그래도 많이 부족한 결과임을 알 수 있었다.
...
* 과제를 수행하며 Logistic Regression, Random Forest, SupportVectorMachine 등 다양한 머신러닝 알고리즘에 대해 탐구하고 활용해 볼 수 있었고, Wav2Vec2.0 을 통해 음성 데이터를 벡터로 변환하는 알고리즘의 원리에 대해서 이해할 수 있었다. 


* 머신러닝 시 데이터를 전처리하는 방법에 따라 성능이 달라지기에 올바른 전처리 과정을 고민하는 것이 매우 중요함을 느꼈다. 또한 컴퓨터 성능으로 인해 epoch를 늘리지 못하여 wav2vec2.0을 이용한 코드의 성능이 좋지 못했던 점이 아쉬웠다.
 
* 음성 인식을 통해 감정을 분류하는 모델은 챗봇 모델, 마케팅, 헬스 케어 등 다양한 산업에서 유용하게 사용될 수 있는 분야이기에 앞으로 주목해야 할 분야다. 언젠가 좋은 기술과 좋은 장비들로 모델의 성능을 개선하여 완벽한 모델을 만들고 싶다.
