...
* 과제를 수행하며 Logistic Regression, Random Forest, SupportVectorMachine 등 다양한 머신러닝 알고리즘에 대해 탐구하고 활용해 볼 수 있었고, Wav2Vec2.0 을 통해 음성 데이터를 벡터로 변환하는 알고리즘의 원리에 대해서 이해할 수 있었습니다. 


* 머신러닝 시 데이터를 전처리하는 방법에 따라 성능이 달라지기에 올바른 전처리 과정을 고민하는 것이 매우 중요함을 느꼈습니다. 또한 컴퓨터 성능으로 인해 epoch를 늘리지 못하여 wav2vec2.0을 이용한 코드의 성능이 좋지 못했던 점이 아쉬웠습니다.
 
* 음성 인식을 통해 감정을 분류하는 모델은 챗봇 모델, 마케팅, 헬스 케어 등 다양한 산업에서 유용하게 사용될 수 있는 분야이기에 앞으로 주목해야 할 분야입니다. 언젠가 좋은 기술과 좋은 장비들로 모델의 성능을 개선하여 완벽한 모델을 만들고 싶습니다.
